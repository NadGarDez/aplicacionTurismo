...
export default class MenuLogin extends Component {
  ...
  constructor(props) {
      super(props);
      this.state = {
        paso: this.props.route.params.estado == 2 ? 3 : 1 ,
        email:"",
        code:""
      };
  ...
  async handleResponse(response){
    ...
    if(!response.error){
      await AsyncStorage.setItem('code', response.code);
      await AsyncStorage.setItem("emailChecked", response.email)
      this.state.paso= 3;
      this.forceUpdate()
